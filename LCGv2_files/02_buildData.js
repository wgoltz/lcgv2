...
function buildJsonObj(init)
{
  ...
  PlotData = TotalData.slice(0)
   ...
   var PlotObsCodes=[]
    ...
    if(PlotData.length>1) //---cannot build plot with 1 data point---
    {
        if(init!=true &&  init!="APASS")
            clearPreviousPlot()
        ...
        for(var k = 0; k<PlotData.length; k++)
        {
           ...
           if(((mtype=="DIFF" && diff==1)||(mtype=="STD" && diff==0)||(mtype=="" && diff==0)))
            if(ob.JD!=""&&ob.val!="T"&&obsCode==true)   //---T is discrepent---
            {
                var band = ob.band
                var re = /\./   //--(Vis.)---
                ...
                if(by!="APASS")//---or other databases---
                {
                    ...
                    if(uncert>0)
                    {
                        ...
                        (
                            {
                            index: jsonCnt-1, band: band, JD: jd, time: julian2Date(jd), mag: mag, uncert: uncert
                            }
                        )
                    }
...
*/
    ...
    {
        sendAnotherMsgDiv.innerHTML = "<center>Observations not found</center>"
    }
}
var PlotJDStart
var PlotJDEnd
